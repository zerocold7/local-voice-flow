"""
Zero- Flow Engine — reader entry point.

    highlight text  →  [read hotkey]  →  clean  →  Kokoro speaks it

Run with `python -m reader` from the repository root, or via Launch_Reader.bat.
"""
import logging
import os
import sys
...
def toggle_suspend(icon, item):
    ...
    if is_suspended:
        voice_engine.interrupt_audio()
    print(f"{ui.C_ACCENT}🔵 [Settings]{ui.C_RESET} Listener {'SUSPENDED' if is_suspended else 'ACTIVE'}")
...
def build_tray_menu():
    ...
    return pystray.Menu(
        MenuItem("Suspend Listener", toggle_suspend, checked=lambda item: is_suspended),
